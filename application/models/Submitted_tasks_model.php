...
class Submitted_tasks_model extends CI_Model{
    ...
    {
        $uuid = $this->db->select("uuid")->from("submitted_tasks")->where("id",$id)->get()->row()->uuid;
        ...
        if(empty($data['uuid'])){
            ...
            foreach($members as $m){
                ...
                $emailData = [
                    ...
                    'link'          =>  base_url('submitted_tasks/view?task_uuid='.$uuid),
    ...
    public function saveNote($data)
    {
        ...
        $taskUuid = $this->db->select("uuid")->from("submitted_tasks")->where("id",$data['task_id'])->get()->row()->uuid;
    ...
    public function bulkChangeStage($taskIds, $stage)
    {
        ...
        $this->db->query("UPDATE submitted_tasks SET stage = '$stage' WHERE id IN ($taskids)");
    }
    ...
    {
        $taskids = implode(',',$taskIds);
        $this->db->query("UPDATE submitted_tasks SET due_date = '$dueDate' WHERE id IN ($taskids)");


        //get task details to email developers
        $submitted_tasks = $this->db->query("SELECT t.name, t.task_number, s.name sprint_name, p.name project_name, c.company_name FROM submitted_tasks t 
                                    JOIN sprints s ON s.id = t.sprint_id
                                    JOIN projects p ON p.id = s.project_id
                                    JOIN customers c ON c.customer_id = p.customer_id
    ...
    public function process_import($sprint_id)
    {
        ...
        $task_count = $this->db->select("MAX(task_number) as n")
                ->from("sprints s")
                ->join("submitted_tasks t","t.sprint_id = s.id","left")
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
            ...
            $maxTN = $this->db->query("SELECT MAX(task_number) as tn FROM submitted_tasks WHERE sprint_id = '$sprint_id' AND status = 1")->row()->tn;
            ...
            $this->db->insert('submitted_tasks',$data);
        }
        fclose($handle);
    }
    ...
    public function assignUsers($userIds,$taskIds,$customerId,$projectId,$sprintId)
    {
        ...
        //fetch all selected submitted_tasks
        $submitted_tasks = $this->Tasks_model->getByIds($taskIds);
        ...
        foreach($userIds as $userId){
            ...
            $user = $this->db->select("email, name")->from("users")->where("id",$userId)->get()->row();
                ...
                $content .= $this->load->view("_email/footer",[], true);
                // echo $content;
                $this->Email_model3->save($user->email,"You have been assigned some submitted_tasks",$content);
        }
    }
