...
class Migration_Create_meeting_notes extends CI_Migration {
    ...
    public function up()
    {
        ...
        $menuQuery = "INSERT INTO `menu` (`id`, `type`, `nom`, `controller`, `action`, `color`, `url`, `class`, `display_order`, `parent_menu`, `visible`, `Normal`, `Admin`, `Root`, `module`, `status`, `backoffice`) VALUES
                    ...
                    (502, 'menu', 'Add', 'meeting_notes', 'create', '', NULL, 'fa-plus-square', 2, 500, 1, 1, 1, 1, 0, 1, 0),
                    ...
                    (506, 'menu', 'PDF', 'meeting_notes', 'pdf', '', NULL, '', 999, 0, 0, 1, 1, 1, 0, 1, 0)";
        $this->db->query($menuQuery);
        
        for($menu_id=500;$menu_id<=506;$menu_id++){
            $var = array("user_id"=>$_SESSION['user_id'],"menu_id"=>$menu_id,"read"=>'1');
            $this->db->insert("permissions",$var);
        }
